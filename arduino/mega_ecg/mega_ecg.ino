...
void setup(){
  ...
  Serial1.begin(38400);
  Serial.begin(115200);  // Baudrate for serial communicate
  //Serial.begin(9600);

  ADS1292.ads1292_Init();  //initialize ADS1292 slave
  //ADS1292.ads1292_Start_Read_Data_Continuous();
  //ADS1292.ads1292_Stop_Read_Data_Continuous();
  //startMillis=millis();
  Serial.println("done");
}
//bool DRDY_STATE;
void loop(){
  ...
  if (ads1292dataReceived == true){      // process the data
    ...
    for (i = 3; i < 9; i += 3)         // data outputs is (24 status bits + 24 bits Respiration data +  24 bits ECG data)
    {
      ...
      uecgtemp = (unsigned long) (uecgtemp << 8);//32bit
      // Serial.println(uecgtemp);
      secgtemp = (signed long) (uecgtemp);
      secgtemp = (signed long) (secgtemp >> 8);
//      UnsignedVal[j++]=uecgtemp; //unsigned 32bit val
    ...
    //currentMillis=millis();
      ...
      //Only remove 40Hz noise, HP (0.67Hz) & LP(100Hz) filter and Kalman filter needed

      //Serial.print("ok");
      Serial1.println(ecg_wave_buff[0]);
    ...
    if(millis() > time_elapsed)  // update every one second
    {
      ...
      else
      {
        ...
        Serial1.print("B");
        Serial1.println(global_HeartRate);
        //Serial1.println("BPM");
      }
      time_elapsed += 1000;
    }
...
//    Serial.println("ok");
//  }
  //Serial.print(DRDY_STATE);
//  ADS1292.ads1292_Read_Data();
//  delay(25);
//  Serial.println(s32DaqVals[1]);
